...
public class CSVEksportkyklop implements CSVEksportkyklopAktig {
    ...
    public void eksporterArrayListTilTekst(String strEntitetsnavn, ArrayList alObjekter) {
        ...
        if (feltListe.isEmpty()) {
            Loggekyklop.hent().loggFEIL("Ingen felter i bean er merket med @SkalEksporteres. Ingen felter blir dermed eksportert, avbryter.");
            return;
        }
    ...
    private String hentVerdier_UtFraFeltListe(Object o) {
        if (feltListe.isEmpty()) {
            Loggekyklop.hent().loggADVARSEL("Du har glemt å oppdatere feltlista. Ikkeno problem, jeg har fiksa for deg. Men det er slurv, altså!");
            byggOppFeltliste(o);
        }
